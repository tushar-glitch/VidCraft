    ...
    if (message && message.Body) {
        ...
        try {
            ...
            if (forward_to_service == 'compressor') {
                // await axios.post(`${process.env.backend_endpoint_compressor}api/v1/compressor`, { bucket, key })
                await axios.post(`http://compressor:6875/api/v1/compressor`, { bucket, key })
            }
            else if (forward_to_service == 'resizer') {
                // await axios.post(`${process.env.backend_endpoint_resizer}api/v1/resizer`, {bucket, key})
                await axios.post(`http://resizer:6876/api/v1/resizer`, { bucket, key })
            }
            else {
